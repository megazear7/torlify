        ...
        ? html`
          <div class="secondary-surface">
            <torlify-auto-textarea
              ...
              @input="${(e: CustomEvent) => (this.bookContext.book!.title = e.detail.value)}"
            ></torlify-auto-textarea>
            <h4>Overview</h4>
            <torlify-auto-textarea
              .value="${this.bookContext.book.overview}"
              @input="${(e: CustomEvent) => (this.bookContext.book!.overview = e.detail.value)}"
            ></torlify-auto-textarea>
            <h4>Edit Instructions</h4>
            <torlify-auto-textarea
              .value="${this.bookContext.book.instructions.edit}"
              @input="${(e: CustomEvent) => (this.bookContext.book!.instructions.edit = e.detail.value)}"
            ></torlify-auto-textarea>
            <h4>Audio Instructions</h4>
            <torlify-auto-textarea
              .value="${this.bookContext.book.instructions.audio}"
              @input="${(e: CustomEvent) => (this.bookContext.book!.instructions.audio = e.detail.value)}"
            ></torlify-auto-textarea>
          </div>
        `
